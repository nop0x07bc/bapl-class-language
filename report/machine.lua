...
-- remaining 56 bit we can pack payloads.
...
Machine.OPCODES.NEWTBL    = 0xB8 -- Pop 1 element TOS(0), create table storage of size TOS(0) and push address to stack.
Machine.OPCODES.GETTBL    = 0xB9 -- Pop 2 elements TOS(0), TOS(1) from the stack and push the contents of table (TOS(1)) at key (TOS(0)) 
Machine.OPCODES.SETTBL    = 0xBA -- Pop 3 elements TOS(0), TOS(1), TOS(2) from the stack, set table (TOS(2)) at key (TOS(1)) to TOS(0).
...
Machine.OPCODES.ISTBL     = 0xF5 -- Pop TOS and Push table test to stack.
Machine.OPCODES.WRITE     = 0xF6 -- Pop 2 elements TOS(0) numbers (bytes) to write can be array or singular number and TOS(1) 
...
Machine.OPCODES.NAME_LOOKUP = {
    ...
    [Machine.OPCODES.NEWTBL]    = "NEWTBL",
    [Machine.OPCODES.GETTBL]    = "GETTBL",
    [Machine.OPCODES.SETTBL]    = "SETTBL",
    [Machine.OPCODES.ISNULL]    = "ISNULL",
    [Machine.OPCODES.ISCLS]     = "ISCLS",
    [Machine.OPCODES.ISARR]     = "ISARR",
    [Machine.OPCODES.ISTBL]     = "ISTBL",
...
function utils.printStep(pc, op, operand, tos, stacksize, cc)
    ...
    elseif type(tos) == "table" and tos.tag == "table" then
        tos_string = string.format("T:%08x", tos.id)
...
function utils.printArray(array, visited)
    ...
    for i = 1,array.size do
        ...
        elseif type(e) == "table" and e.tag == "null" then
            table.insert(parts, "null")
        elseif type(e) == "table" and e.tag == "table" then
            table.insert(parts, string.format("T:%08x", e.id))
        elseif type(e) == "table" and e.tag == "array" then
            local is_loop = false
...
function utils.makeKey(keydata)
    if type(keydata) == "table" and keydata.tag == "null" then
        return ".null"
    elseif type(keydata) == "table" and keydata.tag == "array" then
        local parts = {}
        for _, v in ipairs(keydata) do
...
function Machine:step ()
    ...
    elseif op_variant == Machine.OPCODES.NEWARR then
        ...
        self.pc_ = self.pc_ + 1
    elseif op_variant == Machine.OPCODES.NEWTBL then
        local tos_0 = self.stack_:pop()
        self.stack_:push({size = tos_0, tag = "table", store = {}, id = self.obj_id_})
        self.obj_id_ = self.obj_id_ + 1
        self.pc_ = self.pc_ + 1
    elseif op_variant == Machine.OPCODES.GETARR then
        ...
        self.pc_ = self.pc_ + 1
    elseif op_variant == Machine.OPCODES.GETTBL then
        local tos_0 = self.stack_:pop() -- key
        local tos_1 = self.stack_:pop() -- table
        assert(type(tos_1) == "table" and tos_1.tag == "table", make_error(ERROR_CODES.TYPE_MISMATCH, {message = "Expected table"}))
        
        local key = utils.makeKey(tos_0)

        local value = tos_1.store[key] or {tag = "null"}
    ...
    elseif op_variant == Machine.OPCODES.SETARR then
        ...
        self.pc_ = self.pc_ + 1
    elseif op_variant == Machine.OPCODES.SETTBL then
        local tos_0 = self.stack_:pop() -- value
        local tos_1 = self.stack_:pop() -- key
        local tos_2 = self.stack_:pop() -- table
        assert(type(tos_2) == "table" and tos_2.tag == "table", make_error(ERROR_CODES.TYPE_MISMATCH, {message = "Expected table"}))

        local key = utils.makeKey(tos_1)

        tos_2.store[key] = tos_0
    ...
    elseif op_variant == Machine.OPCODES.PRINT then
        ...
        elseif type(tos_0) == "table" and tos_0.tag == "table" then
            out = string.format("T:%08x", tos_0.id)
    ...
    elseif op_variant == Machine.OPCODES.ISARR then
        ...
        self.pc_ = self.pc_ + 1
    elseif op_variant == Machine.OPCODES.ISTBL then
        local tos_0 = self.stack_:pop()
        self.stack_:push(fromBool(type(tos_0) == "table" and tos_0.tag == "table"))
        self.pc_ = self.pc_ + 1
